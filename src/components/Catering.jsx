    ...
    <div className="catering-page">
      <style>{`
        ...
        .catering-hero-overlay {
          position: absolute; inset: 0;
          background: linear-gradient(180deg, rgba(19,31,61,0.6) 0%, rgba(19,31,61,0.9) 100%);
          z-index: 1;
        }
        ...
        .feature-card {
          position: relative;
          text-align: center; padding: 3rem 1.75rem 2.5rem;
        ...
        .feature-card h3 {
          font-family: 'Cormorant Garamond', serif;
          color: var(--saffron); font-size: 1.55rem;
          margin: 0.75rem 0 0.5rem; font-weight: 600;
        }
        .feature-card p {
          font-family: 'Cormorant Garamond', serif;
          font-size: 1.05rem; line-height: 1.7; color: var(--parchment); margin: 0;
        }
        .feature-icon {
          color: var(--saffron); display: flex; justify-content: center; margin-bottom: 0.5rem;
        }
        ...
        .reservation-title {
          font-family: 'Cormorant Garamond', serif;
          color: var(--saffron); font-size: clamp(1.75rem, 4vw, 2.5rem);
        ...
        .form-group input, .form-group select, .form-group textarea {
          background: rgba(255, 255, 255, 0.08); /* Light background instead of dark */
          border: 1px solid rgba(227, 167, 47, 0.4);
          border-radius: 6px; /* Softer appearance */
          color: var(--ivory); 
        ...
        .form-group input:focus, .form-group select:focus, .form-group textarea:focus {
          ...
          background: rgba(255, 255, 255, 0.12); /* Slightly brighter on focus */
        }
        ...
        .submit-btn:hover { background: var(--saffron-soft); transform: translateY(-2px); }

        .success-banner {
          background: rgba(45, 106, 95, 0.18);
          border: 1px solid rgba(45, 106, 95, 0.6);
          color: #a9d4c8; padding: 1rem; text-align: center;
        ...
        .serving-table th, .serving-table td {
          ...
          border-bottom: 1px solid rgba(227, 167, 47, 0.18);
        ...
        .catering-contact h3 {
          font-family: 'Cormorant Garamond', serif;
          color: var(--saffron); font-size: clamp(1.5rem, 3vw, 2rem);
          margin: 0 0 1rem; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600;
        }
        .catering-contact p {
          font-family: 'Cormorant Garamond', serif; font-style: italic;
          color: var(--parchment); margin: 0 0 1.5rem; font-size: 1.1rem;
        }
      ...
      <section className="catering-features">
        <Feature
          ...
          icon={
            <svg viewBox="0 0 64 64" width="44" height="44" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <path d="M22 32 L42 32 M32 22 L32 42" />
        ...
        <Feature
          ...
          icon={
            <svg viewBox="0 0 64 64" width="44" height="44" fill="none" stroke="currentColor" strokeWidth="1.5">
              <rect x="10" y="22" width="44" height="28" />
        ...
        <Feature
          ...
          icon={
            <svg viewBox="0 0 64 64" width="44" height="44" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <path d="M22 22 L42 22 M22 30 L42 30 M22 38 L36 38" />
            </svg>
          }
        />
      </section>
      ...
      <section className="reservation-card">
        <h2 className="reservation-title">{tx('Request Catering', 'درخواست کترینگ', 'Solicitar Catering')}</h2>
        <p className="reservation-sub">
          {tx(
            'Tell us about your event and we will reply within a day.',
            'درباره‌ی مراسم خود به ما بگویید و ما ظرف یک روز پاسخ می‌دهیم.',
            'Cuéntenos sobre su evento y le responderemos en un día.'
          )}
        </p>

        {submitted && (
          <div className="success-banner">
            ✓ {tx('Request received! We will contact you within 24 hours.', 'درخواست شما دریافت شد! ظرف ۲۴ ساعت با شما تماس می‌گیریم.', '¡Solicitud recibida! Le contactaremos en 24 horas.')}
          </div>
        )}
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label>{tx('Phone Number', 'شماره تماس', 'Teléfono')}</label>
            <input type="tel" name="phone" value={form.phone} onChange={handleChange} required />
          </div>
          <div className="form-group full">
            <label>{tx('Email', 'ایمیل', 'Correo')}</label>
          ...
          <div className="form-group">
            ...
            <input type="number" name="guests" min="10" value={form.guests} onChange={handleChange} required />
          </div>
          <div className="form-group">
            <label>{tx('Preferred Service', 'نوع سرویس', 'Servicio Preferido')}</label>
            <select name="service" value={form.service} onChange={handleChange}>
              <option value="full">{tx('Full Service', 'سرویس کامل', 'Servicio Completo')}</option>
              <option value="dropoff">{tx('Drop-Off', 'تحویل در محل', 'Entrega')}</option>
              <option value="custom">{tx('Custom', 'سفارشی', 'Personalizado')}</option>
            </select>
          </div>
          <div className="form-group full">
            <label>{tx('Notes & Preferences', 'یادداشت و ترجیحات', 'Notas y Preferencias')}</label>
            <textarea name="notes" value={form.notes} onChange={handleChange} />
          </div>
          <button type="submit" className="submit-btn">
            {tx('Submit Request', 'ارسال درخواست', 'Enviar Solicitud')}
          </button>
        </form>
      </section>
      ...
      <div className="view-menu-wrap">
        <Link to="/menu" className="view-menu-btn">
          {tx('View the Menu', 'مشاهده‌ی منو', 'Ver el Menú')}
        </Link>
      </div>

      <section className="serving-guide">
        <h2 className="serving-title">{tx('Serving Guide', 'راهنمای سرو', 'Guía de Servicio')}</h2>
        <p className="serving-note">
          {tx(
            'Recommended tray counts based on guest count. Adjust to taste and dietary mix.',
            'تعداد سینی‌های پیشنهادی بر اساس تعداد مهمانان. بر اساس سلیقه و نیاز تنظیم کنید.',
            'Cantidades recomendadas según el número de invitados. Ajuste según gusto y necesidades.'
          )}
        </p>
          ...
          <table className="serving-table">
            <thead>
              <tr>
                ...
                <th>{tx('Main Trays', 'غذای اصلی', 'Principales')}</th>
      ...
      <section className="catering-contact">
        <h3>{tx('Ready to Plan Your Event?', 'آماده برنامه‌ریزی هستید؟', '¿Listo para Planear?')}</h3>
        <p>{tx('Call or write — we will help you build the perfect table.', 'تماس بگیرید یا بنویسید — کمک می‌کنیم سفره‌ای کامل بچینید.', 'Llámenos o escríbanos — le ayudaremos a armar la mesa perfecta.')}</p>
